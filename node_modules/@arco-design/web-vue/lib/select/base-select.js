...
var dropdownOptgroup = require("../_components/dropdown/dropdown-optgroup.js");
...
var BaseSelect = vue.defineComponent({
  ...
  setup(props, {
    ...
    const computedValue = vue.computed(() => {
      ...
      if (props.multiple) {
        if (!is.isArray(mergedValue)) {
          return mergedValue ? [mergedValue] : [];
        }
      } else if (is.isArray(mergedValue)) {
    ...
    vue.computed(() => utils.getTagDataFromModelValue(computedValue.value, optionInfoMap));
    ...
    const createdOptions = vue.computed(() => {
      const options2 = [];
    ...
    const checkOption = (value) => {
      if (is.isArray(computedValue.value) && enabledOptionSet.has(value)) {
    ...
    vue.watch(computedPopupVisible, (visible) => {
      if (visible) {
        const currentValue = is.isArray(computedValue.value) ? computedValue.value[0] : computedValue.value;
    ...
    const renderOption = (item) => {
      if (item.isGroup) {
        return vue.createVNode(dropdownOptgroup, vue.mergeProps(item._props, {
          "key": item.key,
          "label": item.label
      ...
      const isSelected = is.isArray(computedValue.value) ? computedValue.value.includes(value) : value === computedValue.value;
      return vue.createVNode(dropdownOption, vue.mergeProps(item._props, {
        "ref": (ref) => {
    ...
    return () => vue.createVNode(index, vue.mergeProps(props.triggerProps, {
      "trigger": "click",
      ...
      default: () => [vue.createVNode(selectView, vue.mergeProps(attrs, {
        "class": prefixCls,
        "modelValue": utils.getTagDataFromModelValue(computedValue.value, optionInfoMap),
